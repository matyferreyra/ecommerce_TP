import stock_Cosmetica from "./stock.js";
import carritoFinal from "./stock.js";
...
const mostrarProductos = (cosmetica) => { //1) cosmetica es una variable local de mostrarProductos que pasamos como parámetro. mostrarProductos es nuestro contenedor donde vamos a maquetar nuestros productos que tenemos almacenados en la hoja stock.
    ...
    cosmetica.forEach((stock_Cosmetica) => () { //3) acá hacemos una iteración del array recorriendo cada elemento del array.
        ...
        div.innerHTML += `<div class="description"> 
                                ...
                                <div class="image_product">
                                    <img src="${stock_Cosmetica.img}" alt="${stock_Cosmetica.nombre}"></img>
        ...
        contenedorProductos.appendChild(div); //acá agregamos a la const div de la linea 9 (que es nuestra tarjeta contenedora) como un nodo hijo a ser incorporado. Nuestro nodo hijo va ser toda la tarjeta que se genera a partir de la linea 12 con el innerHTML.
        ...
        carritoFinal(stock_Cosmetica.id) //carritoFinal hace referencia a la página "stock.js"
        alert(`Se agrego ${stock_Cosmetica.cantidad} unidades de ${stock_Cosmetica.nombre} a su compra`) //acá lee la cantidad y el nombre que están guardados en el objeto disponible en la pagina "stock" de JS y lo imprime por pantalla al hacer click al agregar al carrito.
        })
    });
}
...
// function agregarAlCarrito(`${producto.nombre}`) { Esta funcion es la misma que esta en la linea 34. En algún momento hay que optimizarla en el evento citando únicamente al nombre de la función y desarrollando su función en otra hoja del proyecto donde esten todas las funciones.
    // carritoFinal(stock_Cosmetica.id) 
